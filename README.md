...
### 1단계
- 프로덕션
  - [x] 블록구조로 작성되지 않은 코드 수정
  - [x] enum class를 활용하여 state 로직 개선
  - [x] 반복선언된 Color 값에 명칭 부여를 통한 추상화
  - [x] 람다 명칭을 명확하게 수정
  - [x] 파일 내 컴포저블 분리
    - 타 섹션/피쳐에서도 재사용가능성이 높은 컴포저블은 공용 컴포저블로 분리 
- 테스트
  - [x] 테스트 코드 내 공백 표현 수정
  - [x] 에러 메시지 검증 로직 추가
    - enum class 활용을 통한 로직 변경으로 String 기반 공백 문자열 검증의 필요성이 사라졌습니다.